...
export class InternalKeyboardEvent {
    keyDown: boolean
    key: number
    fx: () => void
    modifiers: Modifiers
    constructor(keyDown: boolean, key: number, fx: () => void, modifier: Modifiers) {
...
export class KeyboardHandler {
    ...
    keyEvents: InternalKeyboardEvent[]
    ctrlPressed: boolean
    constructor() {
        this.lastKey = 0
    ...
    defaultKeyDownAction(key : number) {
        this.lastKey = key
    }
    addKeyEvent(keyDown: boolean, key: number, fx: () => void, modifiers: Modifiers) {
        let keyEvent = new InternalKeyboardEvent(keyDown, key, fx, modifiers)
...
export class MouseHandler {
    ...
    onMouseMove(e: MouseEvent) {
        console.log("check mouse move");
        this.updateCoords(e.pageX, e.pageY);
    }
    onMouseDown(e: MouseEvent) {
        console.log("mouse down check")
        switch (e.which) {
            case 1:
                this.cLMB = true;
                this.cRMB = false;
